...
def call_openai(
    ...
    # Dùng max(..., 0.001) để tránh trường hợp mock test trả về latency = 0.0
    latency = max(time.perf_counter() - start_time, 0.001)
...
def streaming_chatbot() -> None:
    ...
    print("=== BẮT ĐẦU STREAMING CHATBOT ===")
...
def run_assistant(
    ...
    while True:
        ...
        try:
            stream = retry_with_backoff(_call_stream, max_retries=3, base_delay=0.1)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    lines = [
        f"{'Prompt':<40} | {'GPT-4o Response':<40} | {'Mini Response':<40} | {'GPT-4o Latency':<12} | {'Mini Latency':<12}",
        "-" * 155,
    ]
    for res in results:
        p = (res['prompt'][:37] + "...") if len(res['prompt']) > 40 else res['prompt']
        g_ans = (res['gpt4o_answer'][:37] + "...") if len(res['gpt4o_answer']) > 40 else res['gpt4o_answer']
        m_ans = (res['mini_answer'][:37] + "...") if len(res['mini_answer']) > 40 else res['mini_answer']
        lines.append(
            f"{p:<40} | {g_ans:<40} | {m_ans:<40} | {res['gpt4o_time']:<12.2f} | {res['mini_time']:<12.2f}"
        )
    return "\n".join(lines)
